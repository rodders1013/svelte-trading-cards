...
import { CARD_WIDTH, CARD_HEIGHT } from '$lib/types/index.js';
...
function buildCompositeSVG(opts: CompositeSVGOptions): string {
	const {
		width,
		height,
		cardSvg,
...
function buildBrandingElements(
	branding: BrandingConfig,
	width: number,
	height: number,
	cardX: number,
	...
	if (branding.logo) {
		...
		elements.push(
			`<image href="${logo.url}" x="${x}" y="${y}" width="${size}" height="${size}" opacity="${opacity}" preserveAspectRatio="xMidYMid meet"/>`
		);
	}
	...
	if (branding.watermark) {
		...
		elements.push(
			`<text x="${x}" y="${y}" text-anchor="${anchor}" font-family="${fontFamily}" font-size="${fontSize}" fill="${color}" opacity="${opacity}">${escapeXml(wm.text)}</text>`
		);
	}
	...
	if (branding.caption) {
		...
		if (cap.position === 'below') {
			...
			if (cap.title) {
				elements.push(
					`<text x="${captionX}" y="${captionY}" text-anchor="middle" font-family="${fontFamily}" font-size="${titleSize}" font-weight="bold" fill="${color}">${escapeXml(cap.title)}</text>`
				);
			...
			if (cap.subtitle) {
				elements.push(
					`<text x="${captionX}" y="${captionY}" text-anchor="middle" font-family="${fontFamily}" font-size="${subtitleSize}" fill="${color}" opacity="0.8">${escapeXml(cap.subtitle)}</text>`
				);
			}
		} else {
			// Right of card
			...
			if (cap.title) {
				elements.push(
					`<text x="${captionX}" y="${captionY}" text-anchor="start" font-family="${fontFamily}" font-size="${titleSize}" font-weight="bold" fill="${color}">${escapeXml(cap.title)}</text>`
				);
			...
			if (cap.subtitle) {
				elements.push(
					`<text x="${captionX}" y="${captionY}" text-anchor="start" font-family="${fontFamily}" font-size="${subtitleSize}" fill="${color}" opacity="0.8">${escapeXml(cap.subtitle)}</text>`
				);
			}
		}
	}
