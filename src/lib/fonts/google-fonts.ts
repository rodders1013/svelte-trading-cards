/**
 * Google Fonts available in the library.
 *
 * These fonts are loaded from Google Fonts CDN on the client,
 * and should have TTF files bundled for server-side rendering.
 *
 * All fonts listed here are free and open source (SIL Open Font License).
 */

import type { FontCategory } from './web-safe.js';

export interface GoogleFont {
	/** Font name as it appears on Google Fonts */
	name: string;
	/** CSS font-family value (with fallback) */
	value: string;
	/** Display label in UI */
	label: string;
	/** Font category for grouping */
	category: FontCategory;
	/** Available weights (for reference) */
	weights: number[];
}

/**
 * Curated list of Google Fonts.
 * These are popular, versatile fonts good for trading cards.
 */
export const GOOGLE_FONTS: GoogleFont[] = [
	// ═══════════════════════════════════════════════════════════════════
	// SANS-SERIF
	// ═══════════════════════════════════════════════════════════════════
	{
		name: 'Roboto',
		value: 'Roboto, sans-serif',
		label: 'Roboto',
		category: 'sans-serif',
		weights: [400, 700]
	},
	{
		name: 'Open Sans',
		value: 'Open Sans, sans-serif',
		label: 'Open Sans',
		category: 'sans-serif',
		weights: [400, 700]
	},
	{
		name: 'Lato',
		value: 'Lato, sans-serif',
		label: 'Lato',
		category: 'sans-serif',
		weights: [400, 700]
	},
	{
		name: 'Montserrat',
		value: 'Montserrat, sans-serif',
		label: 'Montserrat',
		category: 'sans-serif',
		weights: [400, 700]
	},
	{
		name: 'Poppins',
		value: 'Poppins, sans-serif',
		label: 'Poppins',
		category: 'sans-serif',
		weights: [400, 700]
	},
	{
		name: 'Nunito',
		value: 'Nunito, sans-serif',
		label: 'Nunito',
		category: 'sans-serif',
		weights: [400, 700]
	},
	{
		name: 'Raleway',
		value: 'Raleway, sans-serif',
		label: 'Raleway',
		category: 'sans-serif',
		weights: [400, 700]
	},
	{
		name: 'Quicksand',
		value: 'Quicksand, sans-serif',
		label: 'Quicksand',
		category: 'sans-serif',
		weights: [400, 700]
	},
	{
		name: 'Comfortaa',
		value: 'Comfortaa, cursive',
		label: 'Comfortaa',
		category: 'sans-serif',
		weights: [400, 700]
	},
	{
		name: 'Outfit',
		value: 'Outfit, sans-serif',
		label: 'Outfit',
		category: 'sans-serif',
		weights: [400, 700]
	},
	{
		name: 'Lexend',
		value: 'Lexend, sans-serif',
		label: 'Lexend',
		category: 'sans-serif',
		weights: [400, 700]
	},

	// ═══════════════════════════════════════════════════════════════════
	// SERIF
	// ═══════════════════════════════════════════════════════════════════
	{
		name: 'Playfair Display',
		value: 'Playfair Display, serif',
		label: 'Playfair Display',
		category: 'serif',
		weights: [400, 700]
	},
	{
		name: 'Merriweather',
		value: 'Merriweather, serif',
		label: 'Merriweather',
		category: 'serif',
		weights: [400, 700]
	},
	{
		name: 'Lora',
		value: 'Lora, serif',
		label: 'Lora',
		category: 'serif',
		weights: [400, 700]
	},
	{
		name: 'Crimson Text',
		value: 'Crimson Text, serif',
		label: 'Crimson Text',
		category: 'serif',
		weights: [400, 700]
	},

	// ═══════════════════════════════════════════════════════════════════
	// DISPLAY (great for card titles)
	// ═══════════════════════════════════════════════════════════════════
	{
		name: 'Oswald',
		value: 'Oswald, sans-serif',
		label: 'Oswald',
		category: 'display',
		weights: [400, 700]
	},
	{
		name: 'Bebas Neue',
		value: 'Bebas Neue, sans-serif',
		label: 'Bebas Neue',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Anton',
		value: 'Anton, sans-serif',
		label: 'Anton',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Righteous',
		value: 'Righteous, cursive',
		label: 'Righteous',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Bangers',
		value: 'Bangers, cursive',
		label: 'Bangers',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Permanent Marker',
		value: 'Permanent Marker, cursive',
		label: 'Permanent Marker',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Russo One',
		value: 'Russo One, sans-serif',
		label: 'Russo One',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Black Ops One',
		value: 'Black Ops One, cursive',
		label: 'Black Ops One',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Bungee',
		value: 'Bungee, cursive',
		label: 'Bungee',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Orbitron',
		value: 'Orbitron, sans-serif',
		label: 'Orbitron',
		category: 'display',
		weights: [400, 700]
	},
	{
		name: 'Press Start 2P',
		value: 'Press Start 2P, cursive',
		label: 'Press Start 2P',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Audiowide',
		value: 'Audiowide, cursive',
		label: 'Audiowide',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Rajdhani',
		value: 'Rajdhani, sans-serif',
		label: 'Rajdhani',
		category: 'display',
		weights: [400, 700]
	},
	{
		name: 'Exo 2',
		value: 'Exo 2, sans-serif',
		label: 'Exo 2',
		category: 'display',
		weights: [400, 700]
	},
	{
		name: 'Michroma',
		value: 'Michroma, sans-serif',
		label: 'Michroma',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Electrolize',
		value: 'Electrolize, sans-serif',
		label: 'Electrolize',
		category: 'display',
		weights: [400]
	},
	{
		name: 'Teko',
		value: 'Teko, sans-serif',
		label: 'Teko',
		category: 'display',
		weights: [400, 700]
	},
	{
		name: 'Kanit',
		value: 'Kanit, sans-serif',
		label: 'Kanit',
		category: 'display',
		weights: [400, 700]
	},

	// ═══════════════════════════════════════════════════════════════════
	// MONOSPACE
	// ═══════════════════════════════════════════════════════════════════
	{
		name: 'Source Code Pro',
		value: 'Source Code Pro, monospace',
		label: 'Source Code Pro',
		category: 'monospace',
		weights: [400, 700]
	},
	{
		name: 'Fira Code',
		value: 'Fira Code, monospace',
		label: 'Fira Code',
		category: 'monospace',
		weights: [400, 700]
	},
	{
		name: 'JetBrains Mono',
		value: 'JetBrains Mono, monospace',
		label: 'JetBrains Mono',
		category: 'monospace',
		weights: [400, 700]
	},

	// ═══════════════════════════════════════════════════════════════════
	// CURSIVE/SCRIPT
	// ═══════════════════════════════════════════════════════════════════
	{
		name: 'Pacifico',
		value: 'Pacifico, cursive',
		label: 'Pacifico',
		category: 'cursive',
		weights: [400]
	},
	{
		name: 'Dancing Script',
		value: 'Dancing Script, cursive',
		label: 'Dancing Script',
		category: 'cursive',
		weights: [400, 700]
	},
	{
		name: 'Caveat',
		value: 'Caveat, cursive',
		label: 'Caveat',
		category: 'cursive',
		weights: [400, 700]
	},
	{
		name: 'Satisfy',
		value: 'Satisfy, cursive',
		label: 'Satisfy',
		category: 'cursive',
		weights: [400]
	}
];

/**
 * Get Google Fonts formatted for dropdown menus.
 */
export function getGoogleFontOptions(): Array<{ value: string; label: string; category: FontCategory }> {
	return GOOGLE_FONTS.map(f => ({
		value: f.value,
		label: f.label,
		category: f.category
	}));
}

/**
 * Get a Google Font by name.
 */
export function getGoogleFontByName(name: string): GoogleFont | undefined {
	return GOOGLE_FONTS.find(f => f.name === name);
}

/**
 * Get all unique font names (for preloading all fonts).
 */
export function getAllGoogleFontNames(): string[] {
	return GOOGLE_FONTS.map(f => f.name);
}
