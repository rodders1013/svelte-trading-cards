...
export function createInitialCardBackground() {
    return {
        id: 'card-base',
        name: 'Card Base',
        visible: true,
...
export function getComponentByType(container, type) {
    return container.components.find((c) => c.type === type);
}
// Helper to check if component type exists
export function hasComponentType(container, type) {
 ...
 */
function buildPropsFromConfig(comp, propMappings) {
    const props = {};
 ...
 */
function buildBackgroundChildren(comp) {
    const children = [];
    if (comp.fillType === 'solid') {
        ...
        if (comp.animation && comp.animation.type !== 'none')
            props.animation = comp.animation;
        if (comp.holographic)
            props.holographic = comp.holographic;
    ...
    if (comp.patternType && comp.patternType !== 'none') {
        ...
        if (comp.patternSize !== undefined)
            props.size = comp.patternSize;
        if (comp.patternSpacing !== undefined)
            props.spacing = comp.patternSpacing;
        if (comp.patternRotation !== undefined)
            props.rotation = comp.patternRotation;
        if (comp.patternStrokeWidth !== undefined)
            props.strokeWidth = comp.patternStrokeWidth;
        // Single icon pattern
        if (comp.patternType === 'icon' && comp.patternIcon) {
            props.icon = comp.patternIcon;
            props.iconName = comp.patternIconName;
            if (comp.patternIconRotation !== undefined)
                props.iconRotation = comp.patternIconRotation;
            if (comp.patternIconScale !== undefined)
                props.iconScale = comp.patternIconScale;
        }
        // Multi-icon pattern
        if (comp.patternType === 'icons' && comp.patternIcons && comp.patternIcons.length > 0) {
            props.icons = comp.patternIcons;
            if (comp.patternRowOffset !== undefined)
                props.rowOffset = comp.patternRowOffset;
        }
        if (comp.effect)
            props.effect = comp.effect;
        if (comp.blendMode && comp.blendMode !== 'normal')
 ...
 */
function buildBorderProps(comp) {
    const props = {
        color: comp.color,
        width: comp.width,
...
export function buildTemplate(templateName, containers) {
    const components = [];
    for (const container of containers) {
        ...
        for (const comp of container.components) {
            ...
            if (config) {
                ...
                if (config.guard && !config.guard(comp))
                    continue;
                children.push({
                    id: comp.id,
...
export function buildTemplateWithDisplay(templateName, containers, display) {
    ...
    if (display && (display.rarity || display.customGradient)) {
        return {
            ...template,
            display: {
                ...(display.rarity && { rarity: display.rarity }),
...
export function buildPreviewData(previewMode, currentCard, dataFields) {
    if (previewMode === 'data') {
        return currentCard;
    }
...
export function createStatPanelComponent() {
    return {
        type: 'statpanel',
...
export function createProgressBarComponent() {
    return {
        type: 'progressbar',
...
export function createRibbonComponent() {
    return {
        type: 'ribbon',
...
export function createIconRatingComponent() {
    return {
        type: 'iconrating',
...
export function createContainer(containerNumber) {
    return {
        id: generateId(),
