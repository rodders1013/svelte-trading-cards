...
export function hasActiveFilters(config) {
    if (!config)
        return false;
    return ((config.brightness !== undefined && config.brightness !== 1) ||
 ...
 */
export function hasActiveTransform(config) {
    if (!config)
        return false;
    return ((config.offsetX !== undefined && config.offsetX !== 0) ||
 ...
 */
export function buildImageTransform(config, containerWidth, containerHeight) {
    if (!config) {
        return {
            transform: '',
