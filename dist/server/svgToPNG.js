...
function validateSVG(svgString) {
    ...
    if (svgString.length > MAX_SVG_SIZE) {
        throw new SVGValidationError(`SVG exceeds maximum size of ${MAX_SVG_SIZE / 1024 / 1024}MB (received ${(svgString.length / 1024 / 1024).toFixed(2)}MB)`);
    }
    // Complexity validation - count group elements
    const groupCount = (svgString.match(/<g[\s>]/gi) || []).length;
    if (groupCount > MAX_GROUP_COUNT) {
        throw new SVGValidationError(`SVG exceeds maximum complexity (${groupCount} groups, max ${MAX_GROUP_COUNT})`);
    }
}
/**
